...
- `/notes`：我的笔记列表
- `/notes/new`：新建笔记
- `/notes/{note_id}`：笔记详情（完整信息，只读）
- `/notes/{note_id}/write`：笔记写作编辑
...
- 详情页（`/notes/{note_id}`）：
  ...
  - 标题旁用标签文案区分类型：`笔记`
  - 顶部操作包含：
    - `返回`：返回来源 Tab（我的/收藏/广场）
    - `写作`：进入 `/notes/{note_id}/write`
  - 来源信息卡片（URL、标题、作者、发布时间）
  - 公开笔记展示互动区：收藏/点赞图标计数按钮 + 创作者 hover 卡片加关注（样式与条目卡片一致）
  - AI 摘要卡片（只读）
  - 学习心得只读展示
- 写作页（`/notes/{note_id}/write`）：
  - 页面标题为 `写作`
  - 学习心得编辑器（Markdown 文本）
  - 可见性切换（私有/公开）与标签编辑
  - 支持手动保存、重试分析、删除
- 公开页（`/notes/public/{note_id}`）：
  - 只读展示来源信息、AI 摘要、学习心得
  - 不显示编辑与重试按钮
- 信息流详情页（`/feed/items/{item_type}/{item_id}`）：
  - 页面标题统一为 `详情`
  - 标题旁用标签文案区分类型：`聚合` / `笔记`
  - 收藏/点赞使用与条目卡片一致的图标计数按钮（支持直接操作）
  - 加关注使用与条目卡片一致的创作者 hover 卡片交互
