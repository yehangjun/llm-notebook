...
class MimoClient:
    def analyze(
        ...
        *,
        ...
        api_key = (settings.mimo_api_key or "").strip()
        if not api_key:
            raise MimoClientError(code="mimo_not_configured", message="MIMO API Key 未配置")
    ...
    def _request_with_retry(self, *, payload: dict[str, Any], api_key: str) -> dict[str, Any]:
        retries = max(1, settings.mimo_max_retries)
        timeout = max(1, settings.mimo_timeout_seconds)
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                raise MimoClientError(
                    ...
                    message=f"MIMO 请求失败（HTTP {exc.code}）",
                ) from exc
            except urllib.error.URLError as exc:
                if attempt < retries:
                    self._backoff(attempt)
                    continue
                raise MimoClientError(code="mimo_network_error", message="MIMO 网络异常，请稍后重试") from exc
            except TimeoutError as exc:
                if attempt < retries:
                    self._backoff(attempt)
                    continue
                raise MimoClientError(code="mimo_timeout", message="MIMO 请求超时，请稍后重试") from exc
            except json.JSONDecodeError as exc:
                raise MimoClientError(code="mimo_invalid_response", message="MIMO 响应解析失败") from exc

        raise MimoClientError(code="mimo_request_failed", message="MIMO 请求失败")

    def _parse_result(self, response_data: dict[str, Any]) -> MimoAnalysisResult:
        choices = response_data.get("choices")
        if not isinstance(choices, list) or not choices:
            raise MimoClientError(code="invalid_output", message="模型输出为空")
        ...
        if not summary:
            raise MimoClientError(code="invalid_output", message="模型输出缺少有效摘要")
        if not tags:
            raise MimoClientError(code="invalid_output", message="模型输出缺少有效标签")
    ...
    def _parse_json_content(self, text_content: str) -> dict[str, Any]:
        ...
        raise MimoClientError(code="invalid_output", message="模型输出不是有效 JSON")
    ...
    def _chat_endpoint(self) -> str:
        base = (settings.mimo_base_url or "").strip().rstrip("/")
        if not base:
            raise MimoClientError(code="mimo_base_url_not_configured", message="MIMO Base URL 未配置")
        if base.endswith("/v1"):
