...
- Prism 维护一组预设信息源网站，对其做自动信息聚合
  ...
  - 网站列表放到一个配置文件里便于更新维护
  - 涵盖主要的AI博客网站，比如：
    - 英文：Google/Microsoft/OpenAI Research Blog
    - 中文：机器之心
...
- 每个聚合条目都触发大模型内容分析
  ...
  - 记录分析结果、分析时间、模型版本
  - 为了避免开发阶段重复调用大模型浪费token，配置文件里设定一个开关是否开启
...
- 聚合条目 AI 分析状态可见，并符合`待分析`->`分析中`->`成功/失败`的流转
...
### 笔记
- 两个tab：笔记，收藏
- 列表展示来源标题，更新时间，收藏/点赞计数，标签，极简摘要 (AI 摘要 和 学习心得 的二次加工，便于列表显示)
...
### 广场
- 两个tab：关注，未关注
- 列表展示来源标题，创作者，更新时间，收藏/点赞计数，标签，极简摘要
- 详情页展示完整信息
- 支持标签筛选
